...
void reg_grid_t::reset(const bbox_t& size, const vec3i_t& dm) {
  ...
  for (int i = 0; i < nodes.size(); i++) nodes[i].second = false;
}
...
bool reg_grid_t::needClosure(uint x, uint y) {
  ...
  for (; gridIdx.z < dim.z; gridIdx.z++) {
    ...
    if (b && (*this)[gridIdx].first.size() == 0) len++;
  }
...
void reg_grid_t::encloseGrid() {
  ...
  for (gridIdx.x = 0; gridIdx.x < dim.x; gridIdx.x++) {
    for (gridIdx.y = 0; gridIdx.y < dim.y; gridIdx.y++) {
      if (!needClosure(gridIdx.x, gridIdx.y)) continue;

      bool bFill = false;
      uint oneLen = 0;
      for (gridIdx.z = 0; gridIdx.z < dim.z; gridIdx.z++) {
        if ((*this)[gridIdx].first.size() > 0) oneLen++;

        if ((*this)[gridIdx].first.size() == 0) oneLen = 0;

        if ((*this)[gridIdx].first.size() > 0 && oneLen == 1) bFill = !bFill;

        if (bFill) (*this)[gridIdx].second = true;
      }
    }
  }
}
...
AABB_t reg_grid_t::getAABB(const vec3i_t& idx) {
  bbox_t bbox;
  bbox.min = gridSize.min +
             vec3_t(idx.x * cellSize.x, idx.y * cellSize.y, idx.z * cellSize.z);
