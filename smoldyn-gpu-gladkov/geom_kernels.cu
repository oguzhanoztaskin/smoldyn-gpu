...
{
	if(geom.devIndices)
		cudaFree(geom.devIndices);

	if(geom.devIndicesStartEnd)
		cudaFree(geom.devIndicesStartEnd);

	if(geom.devVertices)
		cudaFree(geom.devVertices);

	if(geom.devVerticesWorld)
		cudaFree(geom.devVerticesWorld);

	if(geom.devFaceNormals)
		cudaFree(geom.devFaceNormals);

	if(geom.devHashes)
		cudaFree(geom.devHashes);
}
...
__host__	void	UploadGeometryHashes(geom_node_t& node, reg_grid_t& grid)
{
	...
	cudaMemcpy(node.geom.devHashes, host_has_geom,grid.cellCount()*sizeof(uint), cudaMemcpyHostToDevice);
...
void	UploadIndices(geom_node_t& node, reg_grid_t& grid)
{
	...
	cudaMemcpy(node.geom.devIndices,host_indices,idxCountTotal*sizeof(uint), cudaMemcpyHostToDevice);

	cudaMalloc(&node.geom.devIndicesStartEnd, grid.cellCount()*sizeof(uint2));
	cudaMemcpy(node.geom.devIndicesStartEnd,host_cellStEnd,grid.cellCount()*sizeof(uint2), cudaMemcpyHostToDevice);

	cudaMalloc(&node.geom.devFaceNormals, idxCountTotal/3*sizeof(float3));
	cudaMemcpy(node.geom.devFaceNormals, host_normals, idxCountTotal/3*sizeof(float3), cudaMemcpyHostToDevice);
...
float3	v3tof3(const vec3_t& v)
{
	return make_float3(v.x, v.y, v.z);
}

uint3	v3tof3(const vec3i_t& v)
{
...
__host__	void	CreateGeometryNodeDev(geom_node_t& node, Model& model)
{
	...
	cudaMemcpy(node.geom.devVertices,vertices,vertCount*sizeof(float3), cudaMemcpyHostToDevice);
}

__device__	uint	GetCollisionCellID(const collision_grid_t& grid, float3 pos)
{
	if(pos.x < grid.mmin.x ||
		pos.y < grid.mmin.y ||
		pos.z < grid.mmin.z ||
	...
	return z*grid.dim.x*grid.dim.y +
			y*grid.dim.x +
			x;
}

__device__ bool IsGeometryInCell(float3 cmin, uint* geom_hashes, const collision_grid_t& grid)
{
	float3	p = cmin;
	uint cid = 0xffffffff;
...
__global__	void	TransformGeometryDev(float3*	vert_local, float3*	vert_global, uint vertSize, float4 m0, float4 m1, float4 m2, float4 m3)
{
...
__device__	bool	CollideWithGeometryDev(molecule_t& currm, molecule_t& prevm, const uint3* indices,
												const uint2* indicesStartEnd, const float3*	vertices,
												const float3* normals, uint cellid, const int type, const float prob,
												cudahelp::rand::Xor128Generator& gen)
{
	...
	float3	dir = normalize(make_float3(currm.pos.x-prevm.pos.x, currm.pos.y-prevm.pos.y, currm.pos.z-prevm.pos.z));
	float	t,u,v;
...
__global__	void	ProcessCollReflectiveDev(collision_grid_t grid,
												geom_desc_t geom, molecule_t*	curr, molecule_t*	prev,
												uint numParticles, const uint type, const float prob)
{
	const	uint idx = blockDim.x*blockIdx.x + threadIdx.x;
	...
	do
	{
		...
		if(st_cubid != 0xffffffff)
		{
			if(geom.devHashes[st_cubid] != 0)
				collProcessed = CollideWithGeometryDev(currm, prevm, geom.devIndices, geom.devIndicesStartEnd, geom.devVerticesWorld,
									geom.devFaceNormals, st_cubid, type, prob, gen);
		}

		if(!collProcessed && st_cubid != end_cubid && end_cubid != 0xffffffff && currm.type != -1)
		{
			if(geom.devHashes[end_cubid] != 0)
				collProcessed = CollideWithGeometryDev(currm, prevm, geom.devIndices, geom.devIndicesStartEnd, geom.devVerticesWorld,
									geom.devFaceNormals, end_cubid, type, prob, gen);
		}
...
__host__	void	ProcessCollisions(geom_node_t& node, molecule_t*	curr, molecule_t*	prev, int size)
{
	if(node.moved)
		TransformGeometry(node);
	...
	ProcessCollReflectiveDev<<<numBlocks, numThreads>>>(node.grid, node.geom, curr, prev, size, node.type, node.prob);
}
...
__host__	void	TransformGeometry(geom_node_t& node)
{
	...
	TransformGeometryDev<<<numBlocks, numThreads>>>(node.geom.devVertices, node.geom.devVerticesWorld, node.geom.vertSize,
