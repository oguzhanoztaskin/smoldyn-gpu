...
void radixSort(unsigned int *keys, unsigned int *values, unsigned int *tempKeys,
               unsigned int *tempValues, unsigned int *counters,
               unsigned int *countersSum, unsigned int *blockOffsets,
               CUDPPHandle scanPlan, unsigned int numElements,
               unsigned int keyBits, bool flipBits);

void radixSortFloatKeys(float *keys, unsigned int *values, float *tempKeys,
                        unsigned int *tempValues, unsigned int *counters,
                        unsigned int *countersSum, unsigned int *blockOffsets,
                        CUDPPHandle scanPlan, unsigned int numElements,
...
void radixSortKeysOnly(unsigned int *keys, unsigned int *tempKeys,
                       ...
                       unsigned int *blockOffsets, CUDPPHandle scanPlan,
                       unsigned int numElements, unsigned int keyBits,
                       bool flipBits);

void radixSortFloatKeysOnly(float *keys, float *tempKeys,
                            unsigned int *counters, unsigned int *countersSum,
                            unsigned int *blockOffsets, CUDPPHandle scanPlan,
                            unsigned int numElements, unsigned int keyBits,
                            bool negativeKeys);
...
// are unsigned integers (common case is assumed to be pointers or array
// indices).
//
...
class RadixSort {
 public:  // methods
  ...
  // @param keysOnly true if only keys are to be sorted, false if keys and
  // values
  //
  ...
  void sort(float *keys, unsigned int *values, unsigned int numElements,
            unsigned int keyBits, bool negativeKeys) {
    if (values == 0) {
      radixSortFloatKeysOnly(keys, (float *)mTempKeys, mCounters, mCountersSum,
 ...
 public:                                     // constants
  ...
  // These are static so that extern "C" functions in the .cu file can access
  // them
 ...
 protected:  // methods
  ...
  void initialize(unsigned int numElements, bool keysOnly) {
    ...
    unsigned int numBlocks = ((numElements % (CTA_SIZE * 4)) == 0)
                                 ? (numElements / (CTA_SIZE * 4))
                                 : (numElements / (CTA_SIZE * 4) + 1);
    unsigned int numBlocks2 = ((numElements % (CTA_SIZE * 2)) == 0)
                                  ? (numElements / (CTA_SIZE * 2))
                                  : (numElements / (CTA_SIZE * 2) + 1);

    // Initialize cudapp
    cudppCreate(&mtheCudpp);
