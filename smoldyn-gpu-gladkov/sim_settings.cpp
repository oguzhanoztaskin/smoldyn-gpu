...
settings_t::settings_t()
    : benchmark(false),
      ...
      geometry(true),
      mt(false),
      numParticles(SAMPLES_COUNT),
      maxRunTime(-1.0f),
      density(DSMC_DENSITY),
      fnum(DSMC_FNUM),
      temp(DSMC_T),
      grid_dim(GRID_DIM_X, GRID_DIM_Y, GRID_DIM_Z),
      dt(DSMC_DT),
      streamVelocity(0, 0, 0),
      periodicCondition('n'),
      viscValue(0.75f),
      csv(false),
      shaders(true) {
  boundaries.reset(
      vec3_t(gridBoundaries_x, gridBoundaries_y, gridBoundaries_zMin),
...
void settings_t::readFromFile(const char* filename) {
  ...
  sscanf(grid->first_node("min")->value(), "%f %f %f", &boundaries.min.x,
         &boundaries.min.y, &boundaries.min.z);
  sscanf(grid->first_node("max")->value(), "%f %f %f", &boundaries.max.x,
         &boundaries.max.y, &boundaries.max.z);
  sscanf(grid->first_node("dim")->value(), "%d %d %d", &grid_dim.x, &grid_dim.y,
         &grid_dim.z);
...
void settings_t::print() {
  printf(
      ...
      numParticles, benchmark ? "ON" : "OFF", cpu ? "ON" : "OFF",
      geometry ? "ON" : "OFF", mt ? "ON" : "OFF");

  printf("Grid configuration: <%f %f %f> to <%f %f %f> by <%d %d %d>\n",
         boundaries.min.x, boundaries.min.y, boundaries.min.z, boundaries.max.x,
         boundaries.max.y, boundaries.max.z, grid_dim.x, grid_dim.y,
         grid_dim.z);
  ...
  printf("Stream velocity is: %f %f %f\n", streamVelocity.x, streamVelocity.y,
         streamVelocity.z);
