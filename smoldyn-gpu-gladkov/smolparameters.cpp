...
void	zeroreact_t::calcProb(float	dt, float vol)
{
	prob = rate*dt*vol;
}
...
//TODO: change map to vector to keep order of reactions (as they appear in the file)
void	smolparams_t::calcFirstOrderReactProb()
{
	...
	{
		if(it->second.rate > 0)
			it->second.tau = 1.0f/it->second.rate;
	...
	for(;it != end; ++it)
	{
		...
		for(; b != it; b++)
			if(b->second.reactant == it->second.reactant)
				sum += b->second.prob;
...
void	smolparams_t::calcParameters()//const std::string& path, const std::string& prefix)
{
	uint d = getGridDim();
...
void	smolparams_t::dump(std::ostream&	ostr) const
{
	...
	ostr<<"Boundaries min: "<<boundaries.min.x<<","<<boundaries.min.y<<","<<boundaries.min.z<<"\n";
	ostr<<"Boundaries max: "<<boundaries.max.x<<","<<boundaries.max.y<<","<<boundaries.max.z<<"\n";
	ostr<<"Boundary conditions: "<<boundCond[0]<<","<<boundCond[1]<<","<<boundCond[2]<<"\n";
	ostr<<"Time min: "<<startTime<<" Time max: "<<endTime<<" Timestep: "<<timeStep<<"\n";
	...
	ostr<<"Dumping molmoments: "<<dumpMolMoments<<" molcounts: "<<dumpMolCounts<<"\n";
...
/*
	...
	{
		ostr<<"Set: "<<i<<"\n";
		ostr<<"Count: "<<distr[i].count<<"\nPos: "<<distr[i].pos.x<<","<<distr[i].pos.y<<","<<distr[i].pos.z<<"\n";
		ostr<<"Random pos: "<<distr[i].posRandom[0]<<","<<distr[i].posRandom[1]<<","<<distr[i].posRandom[2]<<"\n";
...
void	firstreact_t::dump(std::ostream&	ostr, const smolparams_t& p) const
{
	...
	ostr<<"Reactant Id: "<<reactant<<" Name: "<<p.getSpiece(reactant).name<<" Tau: "<<tau<<"\n";
}
...
void	secondreact_t::dump(std::ostream&	ostr, const smolparams_t& p) const
{
	...
	ostr<<"Second reactant Id: "<<reactant2<<" Name: "<<p.getSpiece(reactant2).name<<" Binding radius: "<<bindrad2<<"\n";
}
