...
__global__	void	CalcNumberOfParticlesDev(T* mols, uint* offsets, int size, P HashFunc)
{
...
template	<class T, class P>
__global__	void	HashParticlesIntoCellsDev(T* mols, uint* offsets, uint* cellOffsets, uint* indices, int size, P HashFunc)
{
	uint idx = blockDim.x*blockIdx.x + threadIdx.x;
...
{
	if(devCellOffsets)
		cudaFree(devCellOffsets);
}

template	<class	T, class P>
__host__	void	BuildGrid(T* mols, uint* offsets, uint*	indices, uint size, uint numCells, P HashFunc)
{
	...
	CalcNumberOfParticlesDev<<<numBlocks, numThreads>>>(mols, offsets, size, HashFunc);
	...
	HashParticlesIntoCellsDev<<<numBlocks, numThreads>>>(mols, offsets, devCellOffsets, indices, size, HashFunc);
}
