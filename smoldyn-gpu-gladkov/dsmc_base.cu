...
void DSMCSolver::ProcessAndOutputStatistics(dsmc::statistics_t* stat,
                                            float4 collStat, std::ostream& out,
                                            bool printCsv) {
  double tot = 0;
  float3 vel;
  ...
  out << "Simulation time: " << m_state->simTime
      << " Running time: " << m_state->realTime << "\n";

  out << "Cell N  ||  SampNum  ||  Dens  ||          Velocity          ||  "
         "Temperature\n";
  ...
  float cell_volume =
      m_settings.cell_size.x * m_settings.cell_size.y * m_settings.cell_size.z;
  ...
  for (uint i = 0; i < cell_count; i++) {
    ...
    vel = make_float3(s.xyzSum.x / s.numInSample, s.xyzSum.y / s.numInSample,
                      s.xyzSum.z / s.numInSample);

    float uu = vel.x * vel.x + vel.y * vel.y + vel.z * vel.z;

    float tt = m_settings.gas.molmass * (s.velMag / s.numInSample - uu) /
               (3.0f * DSMC_BOLZ);
    ...
    if (printCsv)
      ...
      out << i << "  ||  " << uint(s.numInSample) << "  ||  " << denn
          << "  ||  " << vel.x << "," << vel.y << "," << vel.z << "  ||  " << tt
          << std::endl;
  }
  ...
  const float grid_volume = m_settings.boundaries.get_xsize() *
                            m_settings.boundaries.get_ysize() *
                            m_settings.boundaries.get_zsize();
  ...
  float tColl = 2.0f * m_state->simTime * dens2 * grid_volume * DSMC_PI *
                diam2 * pow(tot / Tref, 0.25) *
                sqrtf(DSMC_BOLZ * Tref / (DSMC_PI * molmas)) / m_settings.fnum;

  out << "\n\nExpected collisions: " << tColl
      << " Ratio: " << tColl / collStat.y << "\n";

  std::cout << "Expected collisions: " << tColl
            << " Actual collisions: " << collStat.y << "\n";
  std::cout << "Collision ratio: " << tColl / collStat.y << "\n";

  out << "Average temperature: " << tot
      << " Reference temperature is: " << m_settings.temp
      << "\nAverage density: " << denn
      << " Reference density: " << m_settings.density << "\n\n"
      << std::endl;
}
...
void DSMCSolver::SaveVelocityField(const std::string& filename) {
  ...
  for (uint i = 0; i < ccount; i++) {
    ...
    draw_line(data, FieldWidth, p1.x * FieldWidth, p1.y * FieldHeight,
              p2.x * FieldWidth, p2.y * FieldHeight, 0);
  }
