...
struct distr_t {
  ...
  int bool3ToInt() const {
    return (posRandom[0] << 2) + (posRandom[1] << 1) + posRandom[2];
  }
};
...
struct species_t {
  ...
  inline int getNumberOfMols() const {
    int count = 0;
    for (size_t i = 0; i < distr.size(); i++) count += distr[i].count;
...
class smolparams_t {
 public:
  ...
  void calcParameters();  // const std::string& path = "", const std::string&
                          // prefix = "");
