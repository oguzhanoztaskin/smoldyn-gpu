...
molecule_state_t::~molecule_state_t() {
  ...
  if (molecules) cudaFree(molecules);
}
...
molecule_state_vbo_t::~molecule_state_vbo_t() {
  ...
  if (prev_mols) cudaFree(prev_mols);
}
...
void molecule_state_vbo_t::copy_prev_mols(molecule_t* curr) {
  if (curr == 0) {
    ...
    cudaMemcpy(prev_mols, cc, maxMolecules * sizeof(molecule_t),
               cudaMemcpyDeviceToDevice);
    unlock_molecules();
  } else
    cudaMemcpy(prev_mols, curr, maxMolecules * sizeof(molecule_t),
               cudaMemcpyDeviceToDevice);
}
