...
void test_streams(int* in, int* out, unsigned int size,
                  unsigned int bChunkSize) {
  ...
  std::cout << "Data size: " << size / 1024 / 1024 * 4
            << " Mb Chunk size: " << bChunkSize / 1024 / 1024 << " Mb "
            << " Elements count " << chunkSize
            << " Chunk count: " << chunksCount << "\n";
...
int do_gpu_streams_test(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp("-streams", argv[i])) useStreams = true;

    if (!strcmp("-count", argv[i])) size = atoi(argv[i + 1]);

    if (!strcmp("-chunkSize", argv[i])) bChunkSize = atoi(argv[i + 1]);
  }
