...
class Benchmarker {
 public:
  ...
  void Start(const std::string& name) {
    std::pair<timers_map_iterator_t, bool> res =
        timers_.insert(timers_value_type(name, TimingStrategy::Create()));
    res.first->second->start();
  }

  void StartAvg(const std::string& name) {
    std::pair<timers_map_iterator_t, bool> res = timers_.insert(
        timers_value_type(name + "@@Avg", TimingStrategy::Create()));
    res.first->second->start();
  }
  ...
  float GetValue(const std::string& name) const {
    ...
    if (cit == values_.end()) throw std::invalid_argument("No value " + name);
    return cit->second;
  }
  ...
  float GetAvgValue(const std::string& name) const {
    ...
    if (it == averages_.end()) throw std::invalid_argument("No value " + name);

    const std::vector<float>& vals = it->second;

    for (int i = 0; i < vals.size(); i++) avg += vals[i];

    if (vals.size() > 1) avg /= vals.size();
  ...
  void Reset(bool vals = true, bool avgs = true) {
    if (vals) values_.clear();

    if (avgs) averages_.clear();
...
#ifdef _MSC_VER
  typedef std::tr1::unordered_map<std::string, typename TimingStrategy::MyType>
      timers_map_t;
  typedef typename std::tr1::unordered_map<
      std::string, typename TimingStrategy::MyType>::iterator
      timers_map_iterator_t;
#else
  typedef std::unordered_map<std::string, typename TimingStrategy::MyType>
      timers_map_t;
  typedef typename std::unordered_map<std::string,
                                      typename TimingStrategy::MyType>::iterator
      timers_map_iterator_t;
#endif
