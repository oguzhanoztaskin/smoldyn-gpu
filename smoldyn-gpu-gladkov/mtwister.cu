...
void InitGPUTwisters(const char* fname, unsigned int seed) {
  ...
  if (!fd) printf("initMTGPU(): failed to open %s\n", fname);
  ...
  mt_struct_stripped* MT =
      (mt_struct_stripped*)malloc(MT_RNG_COUNT * sizeof(mt_struct_stripped));
...
__device__ unsigned int GetRandomIntegerFast(unsigned int tid, mt_state* mts,
...
__device__ float GetRandomFloatFast(unsigned int tid, mt_state* mts,
                                    mt_struct_stripped* config) {
  return ((float)GetRandomIntegerFast(tid, mts, config) + 1.0f) / 4294967296.0f;
}

__device__ void InitTwisterFast(unsigned int tid, mt_state* mts,
                                mt_struct_stripped* config) {
...
__device__ float HybridTausRng(uint4* state) {
  return 2.3283064365387e-10 * (TausStep(&state->x, 13, 19, 12, 4294967294UL) ^
...
__device__ uint HybridTausRngInt(uint4* state) {
  return (TausStep(&state->x, 13, 19, 12, 4294967294UL) ^
          TausStep(&state->y, 2, 25, 4, 4294967288UL) ^
          TausStep(&state->z, 3, 11, 17, 4294967280UL) ^ LCGStep(&state->w));
}
/*
...
/*
__global__	void	GenerateRandomSamplesLCGUint4(uint4*	samples, uint
numSamples)
{
