...
struct gpu_grid_t {
  ...
  gpu_grid_t()
      : cellStartEnd(0), indices(0), hashes(0), gridDim(0), offsets(0) {}
};
...
__global__ void InitSpeciesDev(smolgpu::molecule_t* mol, float3* colors,
                               uint* isAlive, float3 col, int count, int type,
...
__global__ void CalculateDiffusionDev(smolgpu::molecule_t* mol, float* diffstep,
                                      float* gtable, int tsize, int count) {
  uint idx = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void UpdateDiffStepsDev(float* diffsteps, float* diffc, int size,
                                   float dt) {
  uint idx = blockDim.x * blockIdx.x + threadIdx.x;
...
__device__ inline uint3 GetCellID3(float3 pos) {
  ...
  return make_uint3(min(x, g_parameters.dim.x - 1),
                    min(y, g_parameters.dim.y - 1),
                    min(z, g_parameters.dim.z - 1));
}

__device__ inline uint MakeCellID3(uint3 idx) {
  return idx.z * g_parameters.dim.x * g_parameters.dim.y +
         idx.y * g_parameters.dim.x + idx.x;
}
...
__device__ unsigned int GetCellID(float3 pos) {
  ...
  return min(z, g_parameters.dim.z - 1) * g_parameters.dim.x *
             g_parameters.dim.y +
...
__host__ void RunInitSpeciesKernel(const species_t& sp, float3* colorss,
                                   uint* isAlive, float3 color,
                                   molecule_t* mols) {
  molecule_t* mol = mols;
  ...
  for (int i = 0; i < sp.distr.size(); i++) {
    ...
    switch (d) {
      case 7:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
            mol, colors, isAlive, color, count, sp.id, DistributeUniformFast(),
            DistributeUniformFast(), DistributeUniformFast());
        break;
      case 6:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
            mol, colors, isAlive, color, count, sp.id, DistributeUniformFast(),
            DistributeUniformFast(), DistributeConstant(pos.z));
        break;
      case 5:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
            mol, colors, isAlive, color, count, sp.id, DistributeUniformFast(),
            DistributeConstant(pos.y), DistributeUniformFast());
        break;
      case 4:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
            mol, colors, isAlive, color, count, sp.id, DistributeUniformFast(),
            DistributeConstant(pos.y), DistributeConstant(pos.z));
        break;
      case 3:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
            mol, colors, isAlive, color, count, sp.id,
            DistributeConstant(pos.x), DistributeUniformFast(),
            DistributeUniformFast());
        break;
      case 2:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
            mol, colors, isAlive, color, count, sp.id,
            DistributeConstant(pos.x), DistributeUniformFast(),
            DistributeConstant(pos.z));
        break;
      case 1:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
            mol, colors, isAlive, color, count, sp.id,
            DistributeConstant(pos.x), DistributeConstant(pos.y),
            DistributeUniformFast());
        break;
      case 0:
        InitSpeciesDev<<<numBlocks, numThreads>>>(
...
__host__ void InitSpecies(molecule_t* mols, float3* colors, uint* isAlive,
                          const smolparams_t& sp) {
  ...
  UpdateDiffStepsDev<<<numBlocks, numThreads>>>(
      devDiffStep, devDiffC, statesCount, g_cpuParams.timeStep);
}
...
struct is_alive_pred {
  __host__ __device__ bool operator()(const smolgpu::molecule_t& m) {
    return m.type != -1;
  }
};
...
struct mol_type_pred {
  ...
  __host__ __device__ mol_type_pred(int tp) : type(tp) {}
...
struct mols_greater {
  __host__ __device__ bool operator()(const smolgpu::molecule_t& m1,
                                      const smolgpu::molecule_t& m2) {
...
__host__ void RunZeroReactionsKernel(molecule_t* mols, float3* col, int type,
                                     int size, float3 color) {
  int numThreads = 128;
  int numBlocks = GetNumberOfBlocks(numThreads, size);

  InitSpeciesDev<<<numBlocks, numThreads>>>(
      mols, col, 0, color, size, type, DistributeUniformFast(),
      DistributeUniformFast(), DistributeUniformFast());
...
__host__ uint DoZerothReactions(molecule_t* mols, float3* colors) {
  smolparams_t::zeroreact_map_t::const_iterator it =
      g_cpuParams.zeroreact.begin();
  smolparams_t::zeroreact_map_t::const_iterator end =
      g_cpuParams.zeroreact.end();
  ...
  for (; it != end; ++it) {
    ...
    //it->second.name.c_str(),it->second.prob);
...
__global__ void FirstReactionsKernelDev(molecule_t* mol, float3* col,
                                        int intype, int outtype, int p2,
                                        int count, uint* freeIdx, float tresh,
...
__host__ void RunFirstReactionsKernel(molecule_t* mols, float3* col, int size,
                                      const firstreact_t& rc) {
  int numThreads = 128;
  int numBlocks = GetNumberOfBlocks(numThreads, size);
  ...
  if (rc.products[0] != -1) c = g_cpuParams.getSpiece(rc.products[0]).color;
  ...
  FirstReactionsKernelDev<<<numBlocks, numThreads>>>(
      mols, col, rc.reactant, rc.products[0], prod2, size, g_freeIdx, rc.prob,
      c, c2);
}
...
__host__ uint DoFirstReactions(molecule_t* mols, float3* colors, uint size) {
  ...
  smolparams_t::firstreact_map_t::const_iterator it =
      g_cpuParams.firstreact.begin();
  smolparams_t::firstreact_map_t::const_iterator end =
      g_cpuParams.firstreact.end();
  ...
  for (; it != end; ++it) {
    if (it->second.deallocMolecules()) doCompaction = true;
  ...
  if (doCompaction) newSize = RemoveDeadMolecules(mols, colors, newSize);
...
__device__ int FindNearestMolecule(molecule_t* mol, molecule_t& m, int react2,
                                   float bindrad2, uint* indices, uint2* cells,
                                   uint cellid, float& min_dst, int& min_idx) {
  uint2 cellStEnd = cells[cellid];
  int partCount = cellStEnd.y - cellStEnd.x;

  if (partCount < 2) return min_idx;
  ...
  for (int i = cellStEnd.x; i < cellStEnd.y; i++) {
    ...
    if (m2.type == -1 || m2.type != react2) continue;
...
__device__ int FindNearestMoleculeNewHash(molecule_t* mol, molecule_t& m,
                                          int react2, float bindrad2,
                                          uint* indices, uint* offsets,
                                          uint cellid, float& min_dst,
                                          int& min_idx) {
...
/*
__device__	bool	ProcessSecondOrderInCell2(molecule_t*	mol, molecule_t&
m, float3*	col, int react2, uint idx, int p1, int p2, float bindrad2,
float3	color, float3 c2, uint*	indices, uint2* cells, uint3 cell_idx,
cudahelp::rand::Xor128Generator& gen)
{
        ...
        min_idx = FindNearestMolecule(mol, m, react2, bindrad2, indices, cells,
cellid, min_dst, min_idx); #else

        for(int x = max(cell_idx.x-1,0); x <=
min(cell_idx.x+1,g_parameters.dim.x-1); x++) for(int y = max(cell_idx.y-1,0); y
<= min(cell_idx.y+1,g_parameters.dim.y-1); y++) for(int z = max(cell_idx.z-1,0);
z <= min(cell_idx.z+1,g_parameters.dim.z-1); z++)
                        {
                                uint	cellid = GetCellID3(make_uint3(x,y,z));
                                min_idx = FindNearestMolecule(mol, m, react2,
bindrad2, indices, cells, cellid, min_dst, min_idx);
                        }
#endif
...
*/
__global__ void FindNearestMoleculesDev(molecule_t* mol, int r1, int r2,
                                        int count, float bindrad2,
                                        uint* indices, uint2* cells,
                                        int* nearest, const bool useNeighbors) {
  ...
  if (m.type == r1 && m.type != -1)  // || m.type == r2)
  {
    ...
    if (!useNeighbors) {
      ...
      min_idx = FindNearestMolecule(mol, m, react2, bindrad2, indices, cells,
                                    cellid, min_dst, min_idx);
    } else {
      for (int x = max(cell_idx.x - 1, 0);
           x <= min(cell_idx.x + 1, g_parameters.dim.x - 1); x++)
        for (int y = max(cell_idx.y - 1, 0);
             y <= min(cell_idx.y + 1, g_parameters.dim.y - 1); y++)
          for (int z = max(cell_idx.z - 1, 0);
               z <= min(cell_idx.z + 1, g_parameters.dim.z - 1); z++) {
            uint cellid = MakeCellID3(make_uint3(x, y, z));
            min_idx = FindNearestMolecule(mol, m, react2, bindrad2, indices,
                                          cells, cellid, min_dst, min_idx);
...
__global__ void FindNearestMoleculesNewHashDev(molecule_t* mol, int r1, int r2,
                                               int count, float bindrad2,
                                               uint* indices, uint* offsets,
                                               int* nearest,
                                               const bool useNeighbors) {
  const uint idx = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  if (m.type == r1 || m.type == r2) {
    ...
    if (!useNeighbors) {
      ...
      min_idx = FindNearestMoleculeNewHash(mol, m, react2, bindrad2, indices,
                                           offsets, cellid, min_dst, min_idx);
    } else {
      for (int x = max(cell_idx.x - 1, 0);
           x <= min(cell_idx.x + 1, g_parameters.dim.x - 1); x++)
        for (int y = max(cell_idx.y - 1, 0);
          ...
          for (int z = max(cell_idx.z - 1, 0);
            ...
            uint cellid = MakeCellID3(make_uint3(x, y, z));
            min_idx =
                FindNearestMoleculeNewHash(mol, m, react2, bindrad2, indices,
                                           offsets, cellid, min_dst, min_idx);
          }
    }
  }
...
__global__ void SecondReactionsKernelDev2(molecule_t* mol, float3* col, int p1,
                                          int p2, int count, float3 color,
...
/*
__global__	void	SecondReactionsKernelNewDev(molecule_t*	mol, float3*
col, int	r1, int r2, int p1, int p2, int count, uint* freeIdx, float
bindrad2, float3	color, float3 c2, uint*	indices, uint2* cells)
{
        const uint idx = blockDim.x * blockIdx.x + threadIdx.x;
        ...
        if(m.type == r1 || m.type == r2)
        {
                ...
                ProcessSecondOrderInCell2(mol, m, col, react2, idx, p1, p2,
bindrad2, color, c2, indices, cells, cell_idx, gen);
        }
}
*/
__global__ void SecondReactionsKernelDev(molecule_t* mol, float3* col, int r1,
                                         int r2, int p1, int p2, int count,
                                         uint* freeIdx, float bindrad2,
                                         float3 color, float3 c2, uint* indices,
                                         uint2* cells) {
...
void RunSecondReactionsKernel(molecule_t* mols, float3* col, int size,
                              const secondreact_t& rc) {
  float3 c = make_float3(1, 1, 1);
  float3 c2 = c;
  ...
  FindNearestMoleculesDev<<<numBlocks, numThreads>>>(
      mols, rc.reactant, rc.reactant2, size, rc.bindrad2, devGrid.indices,
      devGrid.cellStartEnd, devNearest, g_cpuParams.accuracy > 5);

  SecondReactionsKernelDev2<<<numBlocks, numThreads>>>(
      mols, col, rc.products[0], prod2, size, c, c2, devNearest);

#endif
}

void RunSecondReactionsKernelNewHash(molecule_t* mols, float3* col, int size,
                                     const secondreact_t& rc) {
  float3 c = make_float3(1, 1, 1);
  float3 c2 = c;
  ...
  FindNearestMoleculesNewHashDev<<<numBlocks, numThreads>>>(
      mols, rc.reactant, rc.reactant2, size, rc.bindrad2, devGrid.indices,
      devGrid.offsets, devNearest, g_cpuParams.accuracy > 5);
...
__host__ uint DoSecondReactions(molecule_t* mols, float3* colors, uint size) {
  ...
  smolparams_t::secondreact_map_t::const_iterator it =
      g_cpuParams.secondreact.begin();
  smolparams_t::secondreact_map_t::const_iterator end =
      g_cpuParams.secondreact.end();
  ...
  for (; it != end; ++it) {
    ...
    /*
                    ...
                    {
                            newSize = RemoveDeadMolecules(mols, colors,
       newSize); cudaMemcpy(g_freeIdx, &newSize, sizeof(uint),
       cudaMemcpyHostToDevice);
                    }
    */
    //		cudaMemcpy(&newSize, g_freeIdx, sizeof(uint),
...
__host__ uint DoSecondReactionsNewHash(molecule_t* mols, float3* colors,
                                       uint size) {
  cudaMemcpy(g_freeIdx, &size, sizeof(uint), cudaMemcpyHostToDevice);
...
__global__ void CalculateCellIDsDev(molecule_t* mols, uint* d_hashes,
                                    uint* d_indices, uint size) {
  const uint idx = blockDim.x * blockIdx.x + threadIdx.x;
...
__host__ void CalculateCellIDs(molecule_t* mols, uint* d_hashes,
                               uint* d_indices, uint size) {
  uint numThreads = 128;
  uint numBlocks = GetNumberOfBlocks(numThreads, size);

  CalculateCellIDsDev<<<numBlocks, numThreads>>>(mols, d_hashes, d_indices,
                                                 size);
}

__global__ void FindCellBoundariesDev(uint2* cellStartEnd, uint* hashes,
                                      uint* indices, uint size) {
...
__host__ void FindCellBoundaries(
  ...
  FindCellBoundariesDev<<<numBlocks, numThreads, smemSize>>>(
      cellStartEnd, gridParticleHash, gridParticleIndex, numParticles);
}
...
void HashMolecules(molecule_t* mols, uint size) {
  ...
  FindCellBoundaries(devGrid.cellStartEnd, devGrid.hashes, devGrid.indices,
                     size);
}
...
void HashMoleculesAtomics(molecule_t* mols, uint size) {
  ...
  //__host__	void	BuildGrid(T* mols, uint* offsets, int*	indices, int
  //size, int numCells, P HashFunc)
  ...
  cudahelp::BuildGrid(mols, devGrid.offsets, devGrid.indices, size, numCells,
                      hashFunc);
}
...
__host__ void InitGaussTable() {
  ...
  cudaMemcpy(devGaussTable, cpuGaussTable, g_gaussTableSize * sizeof(float),
             cudaMemcpyHostToDevice);
...
__host__ void InitDiffStates() {
  ...
  cudaMemcpy(devDiffC, cpuDiffTable, statesCount * sizeof(float),
             cudaMemcpyHostToDevice);
...
__host__ void InitSimulationParameters(const smolparams_t& params) {
  ...
  p.bmin = make_float3(params.boundaries.min.x, params.boundaries.min.y,
                       params.boundaries.min.z);
  p.bmax = make_float3(params.boundaries.max.x, params.boundaries.max.y,
                       params.boundaries.max.z);
  ...
  if (cudaSuccess !=
      cudaMemcpyToSymbol(g_parameters, &p, sizeof(smol_gpu_params_t))) {
  ...
  thrust::device_ptr<molecule_t> dev_data_end(devTmpMolecules +
                                              g_cpuParams.max_mol);
...
struct mol_plus_pred {
  __host__ __device__ mol_plus_pred() {}

  __host__ __device__ smolgpu::molecule_t operator()(
      const smolgpu::molecule_t& m1, const smolgpu::molecule_t& m2) {
    molecule_t m;
    m.type = m1.type;

    m.pos = make_float3(m1.pos.x + m2.pos.x, m1.pos.y + m2.pos.y,
                        m1.pos.z + m2.pos.z);
...
struct momentum_plus_pred {
  __host__ __device__ momentum_plus_pred() {}

  __host__ __device__ momentum_t operator()(const momentum_t& m1,
                                            const momentum_t& m2) {
    momentum_t m;
...
struct pos2moment1 {
  __host__ __device__ pos2moment1() {}

  __host__ __device__ momentum_t operator()(const smolgpu::molecule_t& m1) {
    momentum_t m;
    m.m1 = m1.pos;
...
struct calc_momenta {
  ...
  __host__ __device__ calc_momenta(float3 av) : avgPos(av) {}
...
__host__ void CalculateMolcountStatistics(std::vector<molcount_stat_t>& stat,
                                          molecule_t* mols, int actSize) {
  smolparams_t::species_map_t::const_iterator it = g_cpuParams.species.begin();
  smolparams_t::species_map_t::const_iterator end = g_cpuParams.species.end();
...
struct hist_condition_x {
  ...
  __device__ int operator()(float3 pos) { return (pos.x - left.x) / bin_size; }
...
struct hist_condition_y {
  ...
  __device__ int operator()(float3 pos) { return (pos.y - left.y) / bin_size; }
...
struct hist_condition_z {
  ...
  __device__ int operator()(float3 pos) { return (pos.z - left.z) / bin_size; }
...
__device__ bool MolInBox(float3 pos, float3 bmin, float3 bmax) {
  return pos.x > bmin.x && pos.x < bmax.x && pos.y > bmin.y && pos.y < bmax.y &&
         pos.z > bmin.z && pos.z < bmax.z;
}

template <class HistAxis>
__global__ void CalculateCountSpaceDev(uint* hist, molecule_t* mols,
                                       int actSize, int specId, float3 bmin,
                                       float3 bmax, HistAxis axis) {
  const uint idx = blockDim.x * blockIdx.x + threadIdx.x;

  if (idx >= actSize) return;
...
__host__ void CalculateMolCountSpace(std::vector<int>& v, molecule_t* mols,
                                     int actSize, int axis, int bins,
                                     int specId, float3 bmin, float3 bmax) {
  if (g_histSize < bins) {
    if (devHistogram) cudaFree(devHistogram);
  ...
  switch (axis) {
    case 0:
      CalculateCountSpaceDev<<<numBlocks, numThreads>>>(
          devHistogram, mols, actSize, specId, bmin, bmax,
          hist_condition_x(bmin, (bmax.x - bmin.x) / bins));
      break;
    case 1:
      CalculateCountSpaceDev<<<numBlocks, numThreads>>>(
          devHistogram, mols, actSize, specId, bmin, bmax,
          hist_condition_y(bmin, (bmax.y - bmin.y) / bins));
      break;
    case 2:
      CalculateCountSpaceDev<<<numBlocks, numThreads>>>(
          devHistogram, mols, actSize, specId, bmin, bmax,
          hist_condition_z(bmin, (bmax.z - bmin.z) / bins));
      break;
  }
...
__global__ void ProcessEquimolDev(molecule_t* mols, float3* colors, int size,
                                  float3 c1, float3 c2, int spec1, int spec2,
                                  float prob) {
  const uint idx = blockDim.x * blockIdx.x + threadIdx.x;
...
__host__ void ProcessEquimol(molecule_t* mols, float3* colors, int size,
                             int spec1, int spec2, float prob) {
  int numThreads = 128;
  int numBlocks = GetNumberOfBlocks(numThreads, size);
  ...
  ProcessEquimolDev<<<numBlocks, numThreads>>>(mols, colors, size, c1, c2,
                                               spec1, spec2, prob);
}

__host__ void CalculateDiffStatistics(std::vector<diff_stat_t>& stat,
                                      molecule_t* mols, int actSize) {
  smolparams_t::species_map_t::const_iterator it = g_cpuParams.species.begin();
  smolparams_t::species_map_t::const_iterator end = g_cpuParams.species.end();
  ...
  for (int i = 0; it != end; ++it, i++) {
    ...
    d.count = thrust::copy_if(dev_data, dev_data_end, dev_tmp_data,
                              mol_type_pred(d.id)) -
              dev_tmp_data;

    thrust::device_ptr<molecule_t> dev_tmp_data_end(devTmpMolecules + d.count);

    thrust::transform(dev_tmp_data, dev_tmp_data_end, dev_momentum_data,
                      pos2moment1());

    inclusive_scan(dev_tmp_data, dev_tmp_data_end, dev_tmp_data,
                   mol_plus_pred());

    molecule_t m;

    cudaMemcpy(&m, devTmpMolecules + d.count - 1, sizeof(molecule_t),
               cudaMemcpyDeviceToHost);
    ...
    thrust::device_ptr<momentum_t> dev_momentum_data_end(devMomentums +
                                                         d.count);

    thrust::transform(dev_momentum_data, dev_momentum_data_end,
                      dev_momentum_data, calc_momenta(d.avgPos));

    inclusive_scan(dev_momentum_data, dev_momentum_data_end, dev_momentum_data,
                   momentum_plus_pred());

    momentum_t mm;

    cudaMemcpy(&mm, devMomentums + d.count - 1, sizeof(momentum_t),
               cudaMemcpyDeviceToHost);
...
__host__ void DeinitSpecies() {
  if (devMomentums) cudaFree(devMomentums);

  if (devNearest) cudaFree(devNearest);

  if (devTmpMolecules) cudaFree(devTmpMolecules);

  if (devDiffC) cudaFree(devDiffC);

  if (devDiffStep) cudaFree(devDiffStep);

  if (devGaussTable) cudaFree(devGaussTable);

  if (devGrid.cellStartEnd) cudaFree(devGrid.cellStartEnd);

  if (devGrid.indices) cudaFree(devGrid.indices);

  if (devGrid.offsets) cudaFree(devGrid.offsets);

  if (devGrid.hashes) cudaFree(devGrid.hashes);
...
__host__ void ProcessBoundaryConditions(molecule_t* mols, uint molsAlive) {
  float3 bmin =
      make_float3(g_cpuParams.boundaries.min.x, g_cpuParams.boundaries.min.y,
                  g_cpuParams.boundaries.min.z);
  float3 bmax =
      make_float3(g_cpuParams.boundaries.max.x, g_cpuParams.boundaries.max.y,
                  g_cpuParams.boundaries.max.z);

  ProcessBoundaryConditionsLookup(mols, molsAlive, bmin, bmax,
                                  g_cpuParams.boundCond);
}
/*
const int N = 6;
  int A[N] = {1, 4, 2, 8, 5, 7};
...
__host__ uint RemoveDeadMolecules(molecule_t* mols, float3* color, uint size) {
  ...
  uint newSize =
      thrust::remove_if(dev_data, dev_data_end, mol_type_pred(-1)) - dev_data;

  return newSize;
}

__host__ uint CalculateDiffusion(molecule_t* mols, float3* colors,
                                 uint molsAlive) {
  int numThreads = 128;
  int numBlocks = GetNumberOfBlocks(numThreads, molsAlive);

  CalculateDiffusionDev<<<numBlocks, numThreads>>>(
      mols, devDiffStep, devGaussTable, g_gaussTableSize, molsAlive);

  ProcessBoundaryConditions(mols, molsAlive);

  if (g_cpuParams.boundCond[0] == 'a' || g_cpuParams.boundCond[1] == 'a' ||
      g_cpuParams.boundCond[2] == 'a')
...
#define rand30()                                 \
  (((unsigned long int)rand() & 32767UL) << 15 | \
   ((unsigned long int)rand() & 32767UL))
#else
...
float gammaln(float x) { /* Returns natural log of gamma function, partly from
                            Numerical Recipies and part from Math CRC */
  double sum, t;
  int j;
  ...
  if (x == floor(x) && x <= 0)
    sum = DBL_MAX;           // 0 or negative integer
...
int poisrandF(float xm) {
        ...
        if(xm<=0) return 0;
                ...
                do {
                        ...
                        em=floor(em);
                        t=0.9*(1.0+y*y)*exp(em*alxm-gammaln(em+1.0)-g); }
while(randCCF()>t); } return (int) em; }
*/
float gammp(float a, float x) {  // Returns incomplete gamma function, partly
...
void GenerateGaussTable(float* data, int size) { randtableF(data, size, 1); }
