...
Model::Model() : type(0), prob(0) {}

Model::~Model() {
#ifndef MODEL_NO_GRAPHICS
  Reset();
#endif
}
...
void Model::surface_t::CalcNormals() {
  ...
  for (int i = 0; i < vertices.size(); i++) {
    for (int j = 0; j < indices.size(); j += 3) {
      if (indices[j] == i)
        normals[i] +=
            vec3_t::cross(vertices[indices[j + 1]] - vertices[indices[j]],
                          vertices[indices[j + 2]] - vertices[indices[j]]);
      else if (indices[j + 1] == i)
        normals[i] +=
            vec3_t::cross(vertices[indices[j]] - vertices[indices[j + 1]],
                          vertices[indices[j + 2]] - vertices[indices[j + 1]]);
      else if (indices[j + 2] == i)
        normals[i] +=
            vec3_t::cross(vertices[indices[j]] - vertices[indices[j + 2]],
...
void Model::surface_t::CalcFacesNormals() {
  ...
  for (int j = 0; j < indices.size(); j += 3) {
    facesNorm[j / 3] =
        calc_normal(vertices[indices[j]], vertices[indices[j + 1]],
...
Model::surface_t Model::surface_t::merge(
    const std::vector<Model::surface_t>& surfaces, const std::string& name) {
  ...
  if (!surfaces.empty()) {
    ...
    for (int i = 0; i < surfaces.size(); i++) {
      surf.vertices.insert(surf.vertices.begin() + vert_offset,
                           surfaces[i].vertices.begin(),
                           surfaces[i].vertices.end());
      surf.texCoords.insert(surf.texCoords.begin() + vert_offset,
                            surfaces[i].texCoords.begin(),
                            surfaces[i].texCoords.end());
      surf.normals.insert(surf.normals.begin() + vert_offset,
                          surfaces[i].normals.begin(),
                          surfaces[i].normals.end());
      surf.colors.insert(surf.colors.begin() + vert_offset,
                         surfaces[i].colors.begin(), surfaces[i].colors.end());

      vert_offset += surfaces[i].vertices.size();

      surf.indices.insert(surf.indices.begin() + offset,
                          surfaces[i].indices.begin(),
                          surfaces[i].indices.end());

      surf.facesNorm.insert(surf.facesNorm.begin() + offset / 3,
                            surfaces[i].facesNorm.begin(),
                            surfaces[i].facesNorm.end());

      std::for_each(surf.indices.begin() + offset,
                    surf.indices.begin() + offset + surfaces[i].indices.size(),
                    adj_indices(max_idx));
...
template <class T>
GLuint CreateBuffer(const std::vector<T>& data, bool elementArray,
                    bool stat = true) {
  GLuint buffId = 0;
...
void Model::TestAABB(const AABB_t& aabb) {
  ...
  for (int i = 0; i < colors.size(); i++) colors[i].reset(1, 1, 1);
...
bool Model::LoadRaw(const char* fname, bool createBuffers) {
  ...
  if (createBuffers) CreateBuffers();

  return true;
}

void Model::CreateBuffers() {
  for (int i = 0; i < surfaces.size(); i++) surfaces[i].CreateBuffers();
}
...
bool Model::Load(const char* filename, bool calcNormals) {
  ...
  if ((l_file = fopen(filename, "rb")) == NULL) return false;
  ...
  while (ftell(l_file) <
         filelength(fileno(l_file)))  // Loop to scan the whole file
  {
    ...
    switch (l_chunk_id) {
      ...
      //             + 2 x float (mapping coordinates) x (number of mapping
      //             points)
      //             + sub chunks
      //-------------------------------------------
      case 0x4140:
...
void Model::surface_t::RenderNormals() {
  for (int i = 0; i < vertices.size(); i++) {
    glBegin(GL_LINES);
    glVertex3fv(&vertices[i].x);
    vec3_t v = vertices[i] + normals[i] * 0.2f;
    glVertex3fv(&v.x);
    glEnd();
  }
}
...
{
//	matrix4x4	m = matrix4x4::transpose(frame.GetMatrix());

//	glMultMatrixf(&m.m[0][0]);

        for(int i = 0; i < surfaces.size(); i++)
                surfaces[i].RenderNormals();
}
*/
void Model::Render(bool n) {
  matrix4x4 m = matrix4x4::transpose(frame.GetMatrix());
  ...
  for (int i = 0; i < surfaces.size(); i++) {
    surfaces[i].Render();
    if (n) surfaces[i].RenderNormals();
  }
}

void Model::RenderGrid() { DrawRegularGrid(grid); }
...
void Model::Reset() {
  for (int i = 0; i < surfaces.size(); i++) surfaces[i].Reset();
}
...
void Model::MoveModel(const vec3_t& translate) {
  for (int i = 0; i < surfaces.size(); i++) {
    ...
    for (int k = 0; k < vert.size(); k++) vert[k] += translate;
    ...
    for (gridIdx.y = 0; gridIdx.y < gridDim.y; gridIdx.y++)
      for (gridIdx.z = 0; gridIdx.z < gridDim.z; gridIdx.z++) {
        ...
        for (uint i = 0; i < ind.size(); i += 3) {
          if (aabb.tri_overlaps(vert[ind[i]], vert[ind[i + 1]],
                                vert[ind[i + 2]])) {
...
void Model::surface_t::Reset() {
  if (vertBuffer) glDeleteBuffers(1, &vertBuffer);
  ...
  if (colorsBuffer) glDeleteBuffers(1, &colorsBuffer);
