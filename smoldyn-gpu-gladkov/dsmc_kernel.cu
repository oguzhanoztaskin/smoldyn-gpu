...
__device__ __host__ float Gamma(float x) {
  float a = 1.0f;
  float y = x;
  if (y < 1.0f)
    a /= y;
  ...
  return a * (1.0f - 0.5748646 * y + 0.9512363 * y * y - 0.6998588 * y * y * y +
              0.4245549 * y * y * y * y - 0.1010678 * y * y * y * y * y);
}

__device__ uint GetCollisionCellID(float3 pos) {
  if (pos.x < g_collGrid.mmin.x || pos.y < g_collGrid.mmin.y ||
      pos.z < g_collGrid.mmin.z ||

      pos.x > g_collGrid.mmax.x || pos.y > g_collGrid.mmax.y ||
  ...
  return z * g_collGrid.dim.x * g_collGrid.dim.y + y * g_collGrid.dim.x + x;
}
...
__host__ uint* UploadGeometryHashes(reg_grid_t& grid) {
  ...
  cudaMalloc(&geom_hashes, grid.cellCount() * sizeof(uint));
  cudaMemcpy(geom_hashes, host_has_geom, grid.cellCount() * sizeof(uint),
...
void UploadIndices(reg_grid_t& grid) {
  ...
  cudaMemcpy(devIndices, host_indices, idxCountTotal * sizeof(uint),
             cudaMemcpyHostToDevice);

  cudaMalloc(&devIndicesStartEnd, grid.cellCount() * sizeof(uint2));
  cudaMemcpy(devIndicesStartEnd, host_cellStEnd,
             grid.cellCount() * sizeof(uint2), cudaMemcpyHostToDevice);

  cudaMalloc(&devFaceNormals, idxCountTotal / 3 * sizeof(float3));
  cudaMemcpy(devFaceNormals, host_normals, idxCountTotal / 3 * sizeof(float3),
             cudaMemcpyHostToDevice);
...
__global__ void DumpConstantsDev(float3* dump) {
  ...
  dump[5] = make_float3(g_simSettings.grid_dim.x, g_simSettings.grid_dim.y,
                        g_simSettings.grid_dim.z);
  dump[6] = make_float3(g_simSettings.cell_volume, g_simSettings.cells_count,
                        g_simSettings.maxRelVel);
}
...
__host__ void InitSimulationProperties(const settings_t& sett) {
  ...
  gpu_sett.gas_props =
      make_float3(sett.gas.molmass, sett.gas.diameter, sett.gas.Tref);
  gpu_sett.phys_state = make_float3(sett.density, sett.fnum, sett.temp);
  gpu_sett.grid_min = make_float3(sett.boundaries.min.x, sett.boundaries.min.y,
                                  sett.boundaries.min.z);
  gpu_sett.grid_max = make_float3(sett.boundaries.max.x, sett.boundaries.max.y,
                                  sett.boundaries.max.z);
  gpu_sett.cell_size =
      make_float3(sett.cell_size.x, sett.cell_size.y, sett.cell_size.z);
  gpu_sett.grid_dim =
      make_uint3(sett.grid_dim.x, sett.grid_dim.y, sett.grid_dim.z);
  ...
  double rv = sett.gas.diameter * sett.gas.diameter * DSMC_PI *
              sqrtf(sett.temp / 300.0) * 300;

  gpu_sett.maxRelVel = rv;

  gpu_sett.viscValue = sett.viscValue;

  gpu_sett.gammaValue = Gamma(2.5 - gpu_sett.viscValue);

  if (cudaSuccess !=
      cudaMemcpyToSymbol(g_simSettings, &gpu_sett, sizeof(gpu_sett))) {
...
__host__ uint* InitRegularGrid(reg_grid_t& grid, const float* vertices,
  ...
  g->mmin = make_float3(grid.gridSize.min.x, grid.gridSize.min.y,
                        grid.gridSize.min.z);
  g->mmax = make_float3(grid.gridSize.max.x, grid.gridSize.max.y,
                        grid.gridSize.max.z);
  g->dim = make_uint3(grid.dim.x, grid.dim.y, grid.dim.z);
  g->cellSize = make_float3(grid.cellSize.x, grid.cellSize.y, grid.cellSize.z);

  if (cudaSuccess !=
      cudaMemcpyToSymbol(g_collGrid, g, sizeof(collision_grid_t))) {
  ...
  cudaMemcpy(devVertices, vertices, vertCount * sizeof(float3),
             cudaMemcpyHostToDevice);
...
__host__ void DeinitRegularGrid() {
  if (devIndices) cudaFree(devIndices);

  if (devIndicesStartEnd) cudaFree(devIndicesStartEnd);

  if (devVertices) cudaFree(devVertices);

  if (devFaceNormals) cudaFree(devFaceNormals);
}

// gives VMP
__device__ float SampleDensityFunction(unsigned int id, mt_state* mts,
                                       mt_struct_stripped* config, float VMP) {
  // according to Bird's code
  return sqrtf(2 * DSMC_BOLZ * g_simSettings.phys_state.z /
               g_simSettings.gas_props.x *
               (-log(GetRandomFloatFast(
                   id, mts, config))));  //*GetRandomFloatFast(id, mts, config);
}

__device__ float SampleDensityFunction(uint4& state, float VMP) {
  // according to Bird's code
  return sqrtf(2 * DSMC_BOLZ * g_simSettings.phys_state.z /
               g_simSettings.gas_props.x * (-log(HybridTausRng(&state))));
}

__device__ float4 GetRandomVelocity(int tid, mt_state* mts,
                                    mt_struct_stripped* config, float3 s) {
  float vv =
      SampleDensityFunction(tid, mts, config, g_simSettings.phys_state.z);
...
__device__ float4 GetRandomVelocity(uint4& state, float3 s) {
  float vv = SampleDensityFunction(state, g_simSettings.phys_state.z);

  const float tetha = 2.0f * DSMC_PI;

  float4 v;

  v.x = vv * sinf(HybridTausRng(&state) * tetha) + s.x;
  v.y = vv * sinf(HybridTausRng(&state) * tetha) + s.y;
  v.z = vv * sinf(HybridTausRng(&state) * tetha) + s.z;
...
__device__ float4 GetRandomPosition(int tid, mt_state* mts,
                                    mt_struct_stripped* config, float3 cell) {
  float4 v;

  v.x =
      cell.x + GetRandomFloatFast(tid, mts, config) * g_simSettings.cell_size.x;
  v.y =
      cell.y + GetRandomFloatFast(tid, mts, config) * g_simSettings.cell_size.y;
  v.z =
      cell.z + GetRandomFloatFast(tid, mts, config) * g_simSettings.cell_size.z;
  v.w = 1;

  return v;
}

__device__ float4 GetRandomPosition(uint4& state, float3 cell) {
  float4 v;

  v.x = cell.x + HybridTausRng(&state) * g_simSettings.cell_size.x;
  v.y = cell.y + HybridTausRng(&state) * g_simSettings.cell_size.y;
  v.z = cell.z + HybridTausRng(&state) * g_simSettings.cell_size.z;
  v.w = 1;
...
// add slice
__global__ void CreateVectorFieldDev(float3* field, const float4* velocities,
                                     const uint2* cellStartEnd,
                                     const uint* indices, const uint slice) {
  const uint cellid =
      gridDim.x * blockDim.x * slice + threadIdx.x * gridDim.x + blockIdx.x;

  const float3 cell = make_float3(
      g_simSettings.grid_min.x + blockIdx.x * g_simSettings.cell_size.x,
  ...
  float3 p1 = make_float3(cell.x + g_simSettings.cell_size.x / 2.0f,
                          cell.y + g_simSettings.cell_size.y / 2.0f,
                          cell.z + g_simSettings.cell_size.z / 2.0f);

  //	float3	p1 = make_float3(cell.x, cell.y +
  // g_simSettings.cell_size.y/2.0f, cell.z + g_simSettings.cell_size.z/2.0f);
  ...
  field[idx * 2 + 0] = p1;
  field[idx * 2 + 1] =
      make_float3(p1.x + p2.x * g_simSettings.cell_size.x / 2.0f,
                  p1.y + p2.y * g_simSettings.cell_size.y / 2.0f,
                  p1.z + p2.z * g_simSettings.cell_size.z / 2.0f);
  //	field[idx*2+1] = make_float3(p1.x + p2.x*g_simSettings.cell_size.x, p1.y
  //+ p2.y*g_simSettings.cell_size.y/2.0f,  p1.z +
  // p2.z*g_simSettings.cell_size.z/2.0f);
}

__host__ void CreateVectorField(float3* field, const float4* velocities,
                                const uint2* cellStartEnd, const uint* indices,
                                const uint slice) {
  CreateVectorFieldDev<<<g_hostSimGrid.dim.x, g_hostSimGrid.dim.y>>>(
      field, velocities, cellStartEnd, indices, slice);
}
...
__global__ void GenerateUniformVelocitiesDevBlocks(float4* pos, float4* vel,
                                                   float3* colors,
  ...
  const uint tid =
      gridDim.x * gridDim.y * threadIdx.x + blockIdx.y * gridDim.x + blockIdx.x;
  ...
  float3 cell = make_float3(
      g_simSettings.grid_min.x + cellX * g_simSettings.cell_size.x,
      g_simSettings.grid_min.y + cellY * g_simSettings.cell_size.y, 0);
  ...
  for (uint cellZ = 0; cellZ < z_dim; cellZ++) {
    uint idx =
        (gridDim.x * gridDim.y * cellZ + blockIdx.y * gridDim.x + blockIdx.x) *
        partPerCell;
...
__global__ void GenerateUniformVelocitiesDevBlocksNoMT(
    float4* pos, float4* vel, float3* colors, const float3 streamVector,
    uint partPerCell, uint z_dim, uint4* seeds) {
  const uint cellX = blockIdx.x;
  const uint cellY = blockIdx.y;
  ...
  for (uint cellZ = 0; cellZ < z_dim; cellZ++) {
    ...
    for (uint i = threadIdx.x; i < partPerCell; i += blockDim.x) {
      v = GetRandomVelocity(state, streamVector);

      vel[idx + i] = v;
      pos[idx + i] = GetRandomPosition(state, cell);
...
__global__ void GenerateInitialVelocitiesDevBlocksNoMT(
    float4* pos, float4* vel, float3* colors, const float3 streamVector,
    uint partPerCell, uint* geom_hashes, uint z_dim, uint4* seeds) {
  const uint cellX = blockIdx.x;
  const uint cellY = blockIdx.y;
  ...
  for (uint cellZ = 0; cellZ < z_dim; cellZ++) {
    ...
    bool k = IsGeometryInCell(
        cell,
        make_float3(g_simSettings.cell_size.x, g_simSettings.cell_size.y,
                    g_simSettings.cell_size.z),
        geom_hashes);
    ...
    if (!k) {
      ...
      for (uint i = threadIdx.x; i < partPerCell; i += blockDim.x) {
        vel[idx + i] = make_float4(7777777, 7777777, 7777777, 7777777);
        pos[idx + i] = make_float4(7777777, 7777777, 7777777, 7777777);
...
__global__ void GenerateInitialVelocitiesDevBlocks(
    float4* pos, float4* vel, float3* colors, const float3 streamVector,
    uint partPerCell, uint* geom_hashes, uint z_dim) {
  const uint cellX = blockIdx.x;
  const uint cellY = blockIdx.y;
...
__host__ void GenInitialVelsUniform(float4* pos, float4* vel, float3* colors,
  ...
  checkCUDAErrorAndThrow("GenInitialVelsUniformBefore");

  if (mt)
    GenerateUniformVelocitiesDevBlocks<<<dimGrid, partPerCell>>>(
        pos, vel, colors, streamVector, partPerCell, g_hostSimGrid.dim.z);
  else
    GenerateUniformVelocitiesDevBlocksNoMT<<<dimGrid, partPerCell>>>(
        pos, vel, colors, streamVector, partPerCell, g_hostSimGrid.dim.z,
        devSeeds);

  checkCUDAErrorAndThrow("GenInitialVelsUniform");
}

__global__ void SampleConcentrationDev(float* concentration,
                                       const uint2* cellStartEnd) {
  const uint cellid =
      gridDim.x * gridDim.y * threadIdx.x + blockIdx.y * gridDim.x + blockIdx.x;
...
__global__ void SampleConcentrationSlicedDev(float* concentration, uint slice,
                                             const uint2* cellStartEnd) {
  const uint cellid =
      gridDim.x * blockDim.x * slice + threadIdx.x * gridDim.x + blockIdx.x;
  ...
  concentration[threadIdx.x * gridDim.x + blockIdx.x] =
      cellStEnd.y - cellStEnd.x;
}

__host__ void SampleConcentrationSliced(float* concentration, const uint slice,
                                        const uint2* cellStartEnd) {
  SampleConcentrationSlicedDev<<<g_hostSimGrid.dim.x, g_hostSimGrid.dim.y>>>(
      concentration, slice, cellStartEnd);
}
...
__host__ void SampleConcentration(float* concentration,
  ...
  SampleConcentrationDev<<<dimGrid, g_hostSimGrid.dim.z>>>(concentration,
                                                           cellStartEnd);
}

__global__ void SampleVelocitiesSlicedDev(float* velocities, const uint slice,
                                          const float4* vels_data,
                                          const uint2* cellStartEnd,
                                          const uint* indices) {
  const uint cellid =
      gridDim.x * blockDim.x * slice + threadIdx.x * gridDim.x + blockIdx.x;
...
__host__ void SampleVelocitiesSliced(float* velocities, const uint slice,
                                     ...
                                     const uint2* cellStartEnd,
                                     const uint* indices) {
  SampleVelocitiesSlicedDev<<<g_hostSimGrid.dim.x, g_hostSimGrid.dim.y>>>(
      velocities, slice, vels_data, cellStartEnd, indices);
}

__global__ void BuildColorFieldDev(float* data, uchar4* colors, float* minmax,
                                   uint xx, uint yy) {
  float mn = minmax[0];
  float mx = minmax[1];
  ...
  if (!(val < 0.1 || mn == mx || mx < 0.1)) alpha = val / (mx - mn);
...
__host__ void BuildColorField(float* data, uchar4* colors, uint width,
  ...
  thrust::inclusive_scan(dev_data, dev_data_end, output.begin(),
  ...
  BuildColorFieldDev<<<width * pix_x, height * pix_y>>>(data, colors, ptr,
                                                        pix_x, pix_y);
}
...
__host__ float4 GetStatistics() {
  ...
  thrust::device_ptr<float4> dev_ptr_end(devStatistics +
                                         g_hostSimGrid.cells_count);

  dsmc::float4plus pf;

  inclusive_scan(dev_ptr, dev_ptr_end, dev_ptr, pf);

  cudaMemcpy(&stat, devStatistics + g_hostSimGrid.cells_count - 1,
             sizeof(float4), cudaMemcpyDeviceToHost);

  return stat;
}

__global__ void SampleCellStatisticsDev(dsmc::statistics_t* stats,
                                        const float4* velocities,
                                        const uint2* cellStartEnd,
                                        const uint* indices) {
  const uint cellid = blockDim.x * gridDim.x * blockIdx.y +
...
__host__ dsmc::statistics_t* SampleCellStatistics(const float4* velocities,
  ...
  SampleCellStatisticsDev<<<dimGrid, g_hostSimGrid.dim.x>>>(
      devCellStatistics, velocities, cellStartEnd, indices);

  dsmc::statistics_t* stat = 0;

  if (readBack) {
    stat = new dsmc::statistics_t[g_hostSimGrid.cells_count];
    cudaMemcpy(stat, devCellStatistics,
               g_hostSimGrid.cells_count * sizeof(dsmc::statistics_t),
...
__global__ void ComputeBirdCollisionsNoMTDev(
    float4* velocities, float2* maximums, float4* statistics, uint4* seeds,
    const uint2* cellStartEnd, const uint* indices, const float avgConc,
    const float dt) {
  const uint cellid = blockDim.x * gridDim.x * blockIdx.y +
                      blockIdx.x * blockDim.x + threadIdx.x;
  ...
  float fpc =
      (0.5 * partCount * avgConc * g_simSettings.phys_state.y * vmax.x * dt) /
          g_simSettings.cell_volume +
      vmax.y;
  ...
  for (uint i = 0; i < pairsCount; i++) {
...
#if 0
		v *= powf((2.0f*DSMC_BOLZ*g_simSettings.gas_props.z)/(g_simSettings.gas_props.x*mag*mag),g_simSettings.viscValue - 0.5f)/g_simSettings.gammaValue;
#endif
    if (v / vmax.x > HybridTausRng(&seed)) {
      ...
      float c =
          2 * DSMC_PI *
          HybridTausRng(&seed);  // GetRandomFloatFast(cellid, &mts, &config);

      angles = make_float3(b * mag / 2.0f, a * mag * __cosf(c) / 2.0f,
                           a * mag * __sinf(c) / 2.0f);
...
// TODO
__global__ void ComputeBirdCollisionsDev(float4* velocities, float2* maximums,
                                         float4* statistics,
                                         const uint2* cellStartEnd,
                                         const uint* indices,
                                         const float avgConc, const float dt) {
  const uint cellid = blockDim.x * gridDim.x * blockIdx.y +
                      blockIdx.x * blockDim.x + threadIdx.x;
  ...
  for (uint i = 0; i < pairsCount; i++) {
    pair.x =
        cellStEnd.x + GetRandomIntegerFast(cellid, &mts, &config) % partCount;
...
void ComputeBird(float4* velocities, const uint2* cellStartEnd,
                 const uint* indices, const uint avgConc, const float dt,
                 const bool mt) {
  dim3 dimGrid;
  dimGrid.x = g_hostSimGrid.dim.y;
  dimGrid.y = g_hostSimGrid.dim.z;
  ...
  if (mt)
    ComputeBirdCollisionsDev<<<dimGrid, g_hostSimGrid.dim.x>>>(
        velocities, maximums, devStatistics, cellStartEnd, indices, avgConc,
        dt);
  else
    ComputeBirdCollisionsNoMTDev<<<dimGrid, g_hostSimGrid.dim.x>>>(
        velocities, maximums, devStatistics, devSeeds, cellStartEnd, indices,
        avgConc, dt);
}
...
__global__ void InitMaximums(float2* maximum) {
  ...
  v.x = g_simSettings
            .maxRelVel;  // DSMC_PI*DSMC_DIAM*DSMC_DIAM*sqrtf(DSMC_T/300.0)*300;
...
__global__ void InitMaximumsNoMT(float2* maximum, uint4* seeds) {
  const uint cellid =
      gridDim.x * gridDim.y * threadIdx.x + blockIdx.y * gridDim.x + blockIdx.x;

  float2 v;

  uint4 state = seeds[cellid];

  v.x = g_simSettings
            .maxRelVel;  // DSMC_PI*DSMC_DIAM*DSMC_DIAM*sqrtf(DSMC_T/300.0)*300;
  v.y = HybridTausRng(&state);
...
void InitBirdData(uint cellsCount, uint partPerCells, bool mt) {
  ...
  thrust::device_ptr<dsmc::statistics_t> stat_dev_end(devCellStatistics +
                                                      cellsCount);
  ...
  CreateRandomSeedsUint4(&devSeeds, g_hostSimGrid.dim.x * g_hostSimGrid.dim.y *
                                        g_hostSimGrid.dim.z);

  checkCUDAErrorAndThrow("CreateRandomSeedsUint4");
  ...
  if (mt)
    ...
    InitMaximumsNoMT<<<dimGrid, g_hostSimGrid.dim.z>>>(maximums, devSeeds);

  checkCUDAErrorAndThrow("InitMaximums");
}

void DeleteBirdData() {
  if (counts) cudaFree(counts);

  if (maximums) cudaFree(maximums);

  if (devConcentration) cudaFree(devConcentration);

  if (devStatistics) cudaFree(devStatistics);

  if (devCellStatistics) cudaFree(devCellStatistics);

  if (devSeeds) cudaFree(devSeeds);

  if (pairs) cudaFree(pairs);
...
struct DevAverage {
  __device__ float operator()(float v) { return v / g_simSettings.cells_count; }
};
...
namespace dsmc {
struct test_mask {
  __host__ __device__ bool operator()(const uint4 x) {
...
__host__ size_t GenerateInitialVelocities(float4* pos, float4* vel, float3* col,
                                          const float3 streamVector,
  ...
  if (mt)
    GenerateInitialVelocitiesDevBlocks<<<dimGrid, partPerCell>>>(
        pos, vel, col, streamVector, partPerCell, geom_hashes,
        g_hostSimGrid.dim.z);
  else
    GenerateInitialVelocitiesDevBlocksNoMT<<<dimGrid, partPerCell>>>(
        pos, vel, col, streamVector, partPerCell, geom_hashes,
        g_hostSimGrid.dim.z, devSeeds);

  thrust::device_ptr<float4> dev_pos(pos);
  thrust::device_ptr<float4> dev_pos_end(pos + numParticles);

  thrust::device_ptr<float4> res = thrust::remove(
      dev_pos, dev_pos_end, make_float4(7777777, 7777777, 7777777, 7777777));

  uint count = numParticles - (dev_pos_end - res);

  thrust::device_ptr<float4> dev_vel(vel);
  thrust::device_ptr<float4> dev_vel_end(vel + numParticles);

  thrust::remove(dev_vel, dev_vel_end,
                 make_float4(7777777, 7777777, 7777777, 7777777));
...
__device__ bool CollideWithGeometry(float4* pos, float4* vel, float4* old_pos,
                                    const uint3* indices,
...
struct PeriodicZBoundaryCondition {
  __device__ bool operator()(float4* pos, float4* vel, float4 old_pos) const {
    ...
    if (pos->z > g_simSettings.grid_max.z) {
      pos->z = g_simSettings.grid_min.z + BOUNCE_DELTA + pos->z -
               g_simSettings.grid_max.z;
      b = true;
    }

    // top side
    if (pos->y > g_simSettings.grid_max.y) {
      pos->y = g_simSettings.grid_max.y -
               BOUNCE_DELTA;  // - (pos->y-gridBoundaries_w);
    ...
    if (pos->z < g_simSettings.grid_min.z) {
      pos->z = g_simSettings.grid_max.z - BOUNCE_DELTA + pos->z -
               g_simSettings.grid_min.z;
      b = true;
    }
    // bottom side
    if (pos->y < g_simSettings.grid_min.y) {
      pos->y = g_simSettings.grid_min.y +
               BOUNCE_DELTA;  // - (pos->y-gridBoundaries_y);
      vel->y *= -1;
    ...
    if (pos->x < g_simSettings.grid_min.x) {
      pos->x = g_simSettings.grid_min.x +
               BOUNCE_DELTA;  // - (pos->z-gridBoundaries_zMin);
      vel->x *= -1;
    ...
    if (pos->x > g_simSettings.grid_max.x) {
      pos->x = g_simSettings.grid_max.x -
               BOUNCE_DELTA;  // - (pos->z-gridBoundaries_zMax);
      vel->x *= -1;
...
struct PeriodicXBoundaryCondition {
  __device__ bool operator()(float4* pos, float4* vel, float4 old_pos) const {
    ...
    // right side - respawn at left one
    if (pos->x > g_simSettings.grid_max.x) {
      pos->x = g_simSettings.grid_min.x + BOUNCE_DELTA + pos->x -
               g_simSettings.grid_max.x;
      b = true;
    }
    ...
    // left side - respawn at the right one
    if (pos->x < g_simSettings.grid_min.x) {
      pos->x = g_simSettings.grid_max.x - BOUNCE_DELTA + pos->x -
               g_simSettings.grid_min.x;
      b = true;
    }
    // bottom side
    if (pos->y < g_simSettings.grid_min.y) {
    ...
    // far plane
    if (pos->z < g_simSettings.grid_min.z) {
      pos->z = g_simSettings.grid_min.z +
               BOUNCE_DELTA;  // - (pos->z-gridBoundaries_zMin);
      vel->z *= -1;
    ...
    // near plane
    if (pos->z > g_simSettings.grid_max.z) {
      pos->z = g_simSettings.grid_max.z -
               BOUNCE_DELTA;  // - (pos->z-gridBoundaries_zMax);
      vel->z *= -1;
...
struct PeriodicYBoundaryCondition {
  __device__ bool operator()(float4* pos, float4* vel, float4 old_pos) const {
    ...
    // right side - respawn at left one
    if (pos->y > g_simSettings.grid_max.y) {
      pos->y = g_simSettings.grid_min.y + BOUNCE_DELTA + pos->y -
               g_simSettings.grid_max.y;
      b = true;
    }
    ...
    // left side - respawn at the right one
    if (pos->y < g_simSettings.grid_min.y) {
      pos->y = g_simSettings.grid_max.y - BOUNCE_DELTA + pos->y -
               g_simSettings.grid_min.y;
      b = true;
    }
    // bottom side
    if (pos->x < g_simSettings.grid_min.x) {
...
__global__ void IntegrateDev(float4* positions, float4* velocities,
                             float3* colors, float dt, const uint numParticles,
                             const bool useCols, BoundaryConditions BC) {
  uint idx = blockDim.x * blockIdx.x + threadIdx.x;
...
template <class BoundaryConditions>
__global__ void ProcessCollisionsDev(
    float4* positions, float4* velocities, float3* colors, float dt,
    const uint* geom_hashes, const uint3* indices, const uint2* indicesStartEnd,
    const float3* vertices, const float3* normals, const uint numParticles,
    const bool useCols, BoundaryConditions BC) {
  ...
  do {
    ...
    if (st_cubid != 0xffffffff) {
      if (geom_hashes[st_cubid] != 0)
        collProcessed =
            CollideWithGeometry(&pos, &vel, &old_pos, indices, indicesStartEnd,
                                vertices, normals, st_cubid);
    }
...
__host__ void Integrate(float4* positions, float4* velocities, float3* colors,
  ...
  switch (bc) {
    case 'x':
      IntegrateDev<<<numBlocks, numThreads>>>(positions, velocities, colors, dt,
                                              numParticles, col,
                                              PeriodicXBoundaryCondition());
      break;
    case 'y':
      IntegrateDev<<<numBlocks, numThreads>>>(positions, velocities, colors, dt,
                                              numParticles, col,
                                              PeriodicYBoundaryCondition());
      break;
    case 'z':
      IntegrateDev<<<numBlocks, numThreads>>>(positions, velocities, colors, dt,
                                              numParticles, col,
                                              PeriodicZBoundaryCondition());
      break;
    default:
      IntegrateDev<<<numBlocks, numThreads>>>(positions, velocities, colors, dt,
                                              numParticles, col,
                                              NoPeriodicBoundaryCondition());
  }
}
...
__host__ void IntegrateAndProcessCollisions(float4* positions,
                                            ...
                                            const bool col, char bc) {
  uint numThreads = 128;
  uint numBlocks = GetNumberOfBlocks(numThreads, numParticles);

  switch (bc) {
    case 'x':
      ProcessCollisionsDev<<<numBlocks, numThreads>>>(
          positions, velocities, colors, dt, geom_hashes, devIndices,
          devIndicesStartEnd, devVertices, devFaceNormals, numParticles, col,
          PeriodicXBoundaryCondition());
      break;
    case 'y':
      ProcessCollisionsDev<<<numBlocks, numThreads>>>(
...
__device__ unsigned int GetCellID3D(float4 pos) {
  ...
  return min(z, g_simSettings.grid_dim.z - 1) * g_simSettings.grid_dim.x *
             g_simSettings.grid_dim.y +
         min(y, g_simSettings.grid_dim.y - 1) * g_simSettings.grid_dim.x +
         min(x, g_simSettings.grid_dim.x - 1);
}

__global__ void CalculateCellIDsDev(float4* positions, uint* d_hashes,
...
__host__ void CalculateCellIDs(float4* positions, uint* d_hashes,
  ...
  CalculateCellIDsDev<<<numBlocks, numThreads>>>(positions, d_hashes, d_indices,
                                                 numParticles);
}

__global__ void FindCellBoundariesDev(
    uint2* cellStartEnd,
  ...
  if (idx == 0 || hash != sharedHash[threadIdx.x]) {
    ...
    if (idx > 0) cellStartEnd[sharedHash[threadIdx.x]].y = idx;
  }
  ...
  if (useCols) sorted_cols[idx] = old_cols[sortedIndex];
#endif
}
...
__host__ void FindCellBoundaries(
  ...
  FindCellBoundariesDev<<<numBlocks, numThreads, smemSize>>>(
      cellStartEnd, sorted_pos, sorted_vel, sorted_cols, gridParticleHash,
      gridParticleIndex, old_pos, old_vel, old_cols, numParticles, col);
}

#ifndef _MSC_VER

__host__ uint CreateRandomSeeds(uint gDim, uint bDim, uint** seeds,
                                uint& sharedMemBytesPerBlock) {
...
__host__ uint CreateRandomSeedsUint4(uint4** seeds, uint size) {
  ...
  cudaMemcpy(*seeds, &cpuSeeds[0], size * sizeof(uint4),
             cudaMemcpyHostToDevice);
...
__global__ void GenerateRandomSamplesFloat4Warp(uint* seeds, float4* samples,
                                                uint numSamples) {
  /*
          ...
          {
                  sample.x = (WarpStandard_Generate(rngRegs, rngShmem) + 1.0f) /
     4294967296.0f; sample.y = (WarpStandard_Generate(rngRegs, rngShmem) + 1.0f)
     / 4294967296.0f; sample.z = 2.0f*DSMC_PI*((WarpStandard_Generate(rngRegs,
     rngShmem) + 1.0f) / 4294967296.0f); sample.w =
     acosf(2.0f*((WarpStandard_Generate(rngRegs, rngShmem) + 1.0f) /
     4294967296.0f)-1.0f);
...
__global__ void PreProcessVectorFieldDev(float3* field, float width,
                                         float height, float minx, float miny) {
  uint idx = threadIdx.x * gridDim.x + blockIdx.x;
...
__global__ void DumpBirdDataDev(float4* data, const float2* maximums,
                                const uint2* cellStartEnd, const float avgConc,
                                const float dt) {
  const uint cellid = blockDim.x * gridDim.x * blockIdx.y +
                      blockIdx.x * blockDim.x + threadIdx.x;
  uint2 cellStEnd = cellStartEnd[cellid];
...
__host__ void DumpBirdData(const uint2* cellStartEnd, const float avgConc,
  ...
  if (bdata == 0) cudaMalloc(&bdata, cellsCount * sizeof(uint4));
  ...
  DumpBirdDataDev<<<dimGrid, g_hostSimGrid.dim.z>>>(bdata, maximums,
                                                    cellStartEnd, avgConc, dt);

  if (hdata == 0) hdata = new float4[cellsCount];
  ...
  for (uint i = 0; i < cellsCount; i++)
    out << hdata[i].x << " " << hdata[i].y << " " << hdata[i].z << " "
        << hdata[i].w << "\n";
}

void PreProcessVectorField(float3* field, float width, float height, float minx,
                           float miny) {
  PreProcessVectorFieldDev<<<g_hostSimGrid.dim.x, g_hostSimGrid.dim.y>>>(
      field, width, height, minx, miny);
}

__host__ void SortParticlesIndices(uint* hashes, uint* indices, uint size) {
#ifdef NVIDIA_RADIX_SORT

  if (radixSort == 0) radixSort = new nvRadixSort::RadixSort(size);
...
void checkCUDAErrorAndThrow(const char* msg) {
  ...
  sprintf(str, "Cuda error: %s: %s.\n", msg, cudaGetErrorString(err));

  if (cudaSuccess != err) throw std::runtime_error(str);
}
