...
__device__	__host__	float	Gamma(float x)
{
	...
	return a*(1.0f - 0.5748646*y + 0.9512363*y*y - 0.6998588*y*y*y + 0.4245549*y*y*y*y - 0.1010678*y*y*y*y*y);
}
...
__host__	uint*	UploadGeometryHashes(reg_grid_t& grid)
{
	...
	cudaMemcpy(geom_hashes,host_has_geom,grid.cellCount()*sizeof(uint), cudaMemcpyHostToDevice);
...
{
//according to Bird's code
	return sqrtf(2*DSMC_BOLZ*g_simSettings.phys_state.z/g_simSettings.gas_props.x*(-log(HybridTausRng(&state))));
}
...
//	float3	p1 = make_float3(cell.x, cell.y + g_simSettings.cell_size.y/2.0f, cell.z + g_simSettings.cell_size.z/2.0f);
...
//	field[idx*2+1] = make_float3(p1.x + p2.x*g_simSettings.cell_size.x, p1.y + p2.y*g_simSettings.cell_size.y/2.0f,  p1.z + p2.z*g_simSettings.cell_size.z/2.0f);
}
...
__global__	void	GenerateUniformVelocitiesDevBlocksNoMT(float4 *pos, float4* vel, float3* colors, const float3 streamVector, uint partPerCell, uint z_dim, uint4* seeds)
{
	const	uint cellX = blockIdx.x;
	const	uint cellY = blockIdx.y;
...
__global__	void	GenerateInitialVelocitiesDevBlocksNoMT(float4 *pos, float4* vel, float3* colors, const float3 streamVector, uint partPerCell, uint* geom_hashes, uint z_dim, uint4* seeds)
{
	const	uint cellX = blockIdx.x;
	const	uint cellY = blockIdx.y;
...
__host__	void	GenInitialVelsUniform(float4 *pos, float4* vel, float3* colors, const float3 streamVector, uint partPerCell, uint numParticles, bool mt)
{
	dim3 dimGrid;
	...
	if(mt)
		...
		GenerateUniformVelocitiesDevBlocksNoMT<<<dimGrid, partPerCell>>>(pos, vel, colors, streamVector, partPerCell, g_hostSimGrid.dim.z, devSeeds);
...
__host__	size_t	GenerateInitialVelocities(float4 *pos, float4* vel, float3* col, const float3 streamVector, uint partPerCell, uint numParticles, uint* geom_hashes, bool mt)
{
	dim3 dimGrid;
	dimGrid.x = g_hostSimGrid.dim.x;
	dimGrid.y = g_hostSimGrid.dim.y;
	...
	if(mt)
		...
		GenerateInitialVelocitiesDevBlocksNoMT<<<dimGrid, partPerCell>>>(pos, vel, col, streamVector, partPerCell, geom_hashes,g_hostSimGrid.dim.z, devSeeds);
