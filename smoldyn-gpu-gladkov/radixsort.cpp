...
inline size_t multiple(size_t x, size_t f) { return ((x + (f - 1)) / f); }

// MS Excel-style CEIL() function
// Rounds x up to nearest multiple of f
inline size_t ceiling(size_t x, size_t f) { return multiple(x, f) * f; }

void computeNumCTAs(KernelPointer kernel, int smemDynamicBytes,
                    bool bManualCoalesce) {
  ...
  if (blockRegisterAllocation) {
    ...
    regsPerCTA =
        ceiling(attr.numRegs * devprop.warpSize * numWarps, regAllocationUnit);
  } else {
    // warp register allocation
    // Number of regs is regs per thread times warp size, rounded up to multiple
    // of register allocation unit size, times number of warps.
    regsPerCTA =
        ceiling(attr.numRegs * devprop.warpSize, regAllocationUnit) * numWarps;
  }
  ...
  size_t ctaLimitRegs =
      regsPerCTA > 0 ? devprop.regsPerBlock / regsPerCTA : maxBlocksPerSM;
  size_t ctaLimitSMem =
      smemPerCTA > 0 ? devprop.sharedMemPerBlock / smemPerCTA : maxBlocksPerSM;
