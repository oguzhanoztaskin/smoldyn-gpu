...
void simulation_state_t::incSlice(uint dimz) {
  if (slice < dimz - 1) slice++;
}
void simulation_state_t::decSlice() {
  if (slice > 0) slice--;
}
...
void DrawAABB(const AABB_t& aabb) { DrawBBox(bbox_t::from_aabb(aabb)); }
...
void DrawSimulationGrid(const bbox_t& gbbox, const vec3i_t& dim, int slice) {
  ...
  vec3_t cellSize((gbbox.max.x - gbbox.min.x) / dim.x,
                  (gbbox.max.y - gbbox.min.y) / dim.y,
                  (gbbox.max.z - gbbox.min.z) / dim.z);
  ...
  for (; gridIdx.z < gridDim.z; gridIdx.z++)
    for (gridIdx.x = 0; gridIdx.x < gridDim.x; gridIdx.x++)
      for (gridIdx.y = 0; gridIdx.y < gridDim.y; gridIdx.y++) {
        mn.reset(gbbox.min.x + cellSize.x * gridIdx.x,
                 gbbox.min.y + cellSize.y * gridIdx.y,
                 gbbox.min.z + cellSize.z * gridIdx.z);
...
uint CreateTexture(unsigned int size_x, unsigned int size_y) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, size_x, size_y, 0, GL_RGB,
