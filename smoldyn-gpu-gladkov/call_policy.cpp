...
{
public:
	interval_policy(std::istringstream& ss)
	{
		ss>>start>>end>>period;
	}
	...
	{
		out<<"Interval call policy.\nInterval: "<<start<<" "<<end<<" Period: "<<period<<"\n";
	}
...
{
public:
	period_policy(std::istringstream& ss)
	{
		ss>>period;
	}
...
{
public:
	at_time_policy(std::istringstream& ss)
	{
		ss>>tm;
	}
...
public:
	...
	void	dump(std::ostream& out)
	{
		out<<"Progression interval call policy.\nInterval: "<<start<<" "<<end<<" Period: "<<period<<" Progression: "<<xt<<"\n";
	}
private:
	float xt;
	float dxt;
...
call_policy*	CreateCallPolicy(std::istringstream& ss, const smolparams_t& p)
{
	...
	if(name == "j")
		return new interval_policy(ss);

	if(name == "x")
		return new progression_policy(ss);

	if(name == "b")
		return new before_policy(ss);

	if(name == "a")
		return new after_policy(ss,p);

	if(name == "i")
		return new float_interval_policy(ss);

	if(name == "@")
		return new at_time_policy(ss);

	if(name == "n")
		return new period_policy(ss);

	if(name != "e")
	{
		LogFile::get()<<"Unsupported call policy: "<<name<<" command will be executed each iteration\n";
		LogFile::flush();
	}
