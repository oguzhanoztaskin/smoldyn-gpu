...
static const float cellSize_x =
    (gridBoundaries_z - gridBoundaries_x) / GRID_DIM_X;
static const float cellSize_y =
    (gridBoundaries_w - gridBoundaries_y) / GRID_DIM_Y;
...
bool IsLinesIntersect(float2 p1, float2 p2, float2 p3, float2 p4) {
  ...
  if (pN.x >= r_min.x && pN.x <= r_max.x && pN.y >= r_min.y && pN.y <= r_max.y)
    return true;
...
bool IsInsideModel(float2 pos, float* model, int vert_count) {
  ...
  for (int i = 0; i < vert_count - 1; i++) {
    ...
    if (IsLinesIntersect(p1, p2, p3, p4)) c++;
  }
...
void ResetPositionsCPU(float* d_pos, unsigned int count) {
  for (unsigned int i = 0; i < count * 2; i++) d_pos[i] = 0;
}
...
void IntegrateCPU(float* d_vels, float* d_pos, float* d_cols, float dt,
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (npos.x > gridBoundaries_z) {
      d_pos[i * 2 + 0] =
          gridBoundaries_x + BOUNCE_DELTA + npos.x - gridBoundaries_z;
    }

    if (npos.y > gridBoundaries_w) {
      d_pos[i * 2 + 0] += npos.x - old_pos;
      d_pos[i * 2 + 1] =
          gridBoundaries_w - BOUNCE_DELTA - (npos.y - gridBoundaries_w);
      d_vels[i * 2 + 1] *= -1;
    }

    if (npos.x < gridBoundaries_x) {
      d_pos[i * 2 + 0] =
          gridBoundaries_z + BOUNCE_DELTA + npos.x - gridBoundaries_x;
      d_vels[i * 2 + 0] *= -1;
    }
    if (npos.y < gridBoundaries_y) {
      d_pos[i * 2 + 0] += npos.x - old_pos;
      d_pos[i * 2 + 1] =
          gridBoundaries_y + BOUNCE_DELTA - (npos.y - gridBoundaries_y);
      d_vels[i * 2 + 1] *= -1;
    }
...
void UpdateGridCPU(float* d_pos, unsigned int numParticles) {
  for (int j = 0; j < N_GRID_CELLS; j++) g_hashTable[j].clear();
    ...
    for (int j = 0; j < GRID_DIM_X; j++) {
      ...
      for (unsigned int ii = 0; ii < part_in_cell; ii++) {
        ...
        if (GetRandomFloat(mt_id) >= p) {
          ...
          if (i1 != j)
            ComputePostCollisionVel1(mt_id, &d_speeds[i1 * 2],
                                     &d_speeds[j * 2]);
        }
      }
    }
...
float MaxwellDistr(float v) {
  return pow(DSMC_mm / (2 * DSMC_PI * DSMC_K * DSMC_T), 3 / 2.0) * 4 * DSMC_PI *
         v * v * exp(-DSMC_mm * v * v / (2 * DSMC_K * DSMC_T));
}

void GetRandomPosition(unsigned int id, float* pos) {
  pos[0] = gridBoundaries_x +
           GetRandomFloat(id) * (gridBoundaries_z - gridBoundaries_x);
  pos[1] = gridBoundaries_y +
           GetRandomFloat(id) * (gridBoundaries_w - gridBoundaries_y);
}

void GetRandomPositionCell(unsigned int id, float* pos, float x, float y,
                           float with, float height) {
  pos[0] = x + GetRandomFloat(id) * with;
  pos[1] = y + GetRandomFloat(id) * height;
}

void GenerateInitialSpeedsPerCellCPU(float* vels, float* pos, float sx,
                                     float sy, unsigned int partPerCell,
  ...
  for (int i = 0; i < GRID_DIM_Y; i++)
    for (int j = 0; j < GRID_DIM_X; j++) {
      ...
      for (int k = 0; k < partPerCell; k++) {
        ...
        GetRandomPositionCell(idx, &pos[idx * 2], cell_x, cell_y, cellSize_x,
                              cellSize_y);
      }
    }
}
...
float SampleDensityFunction(unsigned int id) {
  ...
  for (;;) {
    ...
    if (MaxwellDistr(n1) <= x2) return n1;
  }
}
...
void GetRandomSpeed(unsigned int id, float* v) {
  ...
  v[0] = vv * cos(GetRandomFloat(id) * 2 * DSMC_PI) *
         sin(GetRandomFloat(id) * DSMC_PI);
  v[1] = vv * sin(GetRandomFloat(id) * 2 * DSMC_PI) *
         sin(GetRandomFloat(id) * DSMC_PI);
}
// bug here
/*
...
void ComputePostCollisionVel1(unsigned int id, float* v1, float* v2) {
  ...
  float mag = sqrtf((v1[0] - v2[0]) * (v1[0] - v2[0]) +
                    (v1[1] - v2[1]) * (v1[1] - v2[1])) /
              2;
