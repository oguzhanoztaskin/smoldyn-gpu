...
void reshape(int w, int h) {
  ...
  vec3_t mn(g_settings.boundaries.min.x, g_settings.boundaries.min.y,
            g_settings.boundaries.min.z);
  vec3_t mx(g_settings.boundaries.max.x, g_settings.boundaries.max.y,
            g_settings.boundaries.max.z);
...
void display() {
  ...
  if (g_simState.paused) DrawText2D(300, 20, "Simulation is paused");
...
void mouse(int btn, int state, int x, int y) {
  if (g_webService) return;

  if (state == GLUT_DOWN) {
    if (btn == GLUT_LEFT_BUTTON) g_mouse_state.leftBtnPressed = true;
    if (btn == GLUT_RIGHT_BUTTON) g_mouse_state.rightBtnPressed = true;

    g_mouse_state.pos_x = x;
    g_mouse_state.pos_y = y;
  } else {
    if (btn == GLUT_LEFT_BUTTON) g_mouse_state.leftBtnPressed = false;
    if (btn == GLUT_RIGHT_BUTTON) g_mouse_state.rightBtnPressed = false;

    g_mouse_state.pos_x = -1;
    g_mouse_state.pos_y = -1;
  }

  if (btn == 3) position.z += 0.5f;

  if (btn == 4) position.z -= 0.5f;
}
...
void update() {
  ...
  if (g_webService) {
    if ((startSimTime >= g_interval ||
         g_simState.simTime - g_settings.startTime == 2 * g_settings.timeStep ||
         g_simState.simTime >= g_settings.endTime) &&
        g_interval != -1) {
      startSimTime = 0;
      char buf[255];
...
void keyboard(unsigned char key, int x, int y) {
  ...
  if (key == 27) throw std::runtime_error("User pressed ESC");

  if (key == ' ') g_simState.togglePause();
  if (key == '+') position.z++;
  if (key == '-') position.z--;

  if (key == 'w') wiredFrame = !wiredFrame;

  if (key == 'n') solver->ToggleRenderNormals();

  if (key == 'g') solver->ToggleRenderGrid();
}
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (argc > 2 && std::string(argv[2]) == "-web") {
      ...
      if (argc > 3) g_interval = atoi(argv[3]);

      if (argc > 4) g_prefix = argv[4];
    }
    ...
    position.x =
        -(g_settings.boundaries.max.x + g_settings.boundaries.min.x) / 2;
    position.y =
        -(g_settings.boundaries.max.y + g_settings.boundaries.min.y) / 2;
    position.z =
        -4 * (g_settings.boundaries.max.z - g_settings.boundaries.min.z) / 2;
  ...
  LogFile::get() << "Simulation run for " << g_simState.realTime
                 << " seconds\n";
