...
public:
	...
	{
		out<<"Cmd: molmoments. Dumping "<<p.getSpiece(specId).name<<" to "<<filename<<"\n";
	}
...
//molcountspace name[(state)] axis low high bins low high low high average filename
...
public:
	...
	void	dump(std::ostream& out, const smolparams_t& p) const
	{
		...
		out<<"Axis is: "<<axis<<" Spieces id: "<<specId<<" bins: "<<bins<<" averages: "<<avg<<"\n";
	...
	void	process(SmoldynSolver& sl)
	{
		...
		const std::vector<int>&	svec = sl.GetMolCountSpace(axis, bins, specId, bmin, bmax);
...
public:
	...
	void	process(SmoldynSolver& sl)
	{
		sl.ProcessEquimol(spec1, spec2, prob);
	}
